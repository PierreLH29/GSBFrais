	...
	*/
	public function _remap($action, $params = array())
	{
		...
		else
		{
			...
			elseif ($action == 'AccepterFiche') 	// AccepterFiche demandé : on active la fonction AccepterFiche du modèle visiteur ...
			{	// TODO : contrôler la validité du second paramètre (mois de la fiche à modifier)
				...
				$this->a_visiteur->AccepterFiche($idVisiteur, $mois,$idFiche);
				// ... et on revient à mesFiches
				$this->a_visiteur->mesFiches($idVisiteur, "La fiche $mois a été acceptée.");
			}
			
			elseif ($action == 'MotifRefus') 	// RefuserFiche demandé : on active la fonction signeFiche du modèle visiteur ...
			{	// TODO : contrôler la validité du second paramètre (mois de la fiche à modifier)
				...
				$this->session->set_userdata('idFiche', $idFiche);//Place l'id de la fiche dans la SESSION
				$this->session->set_userdata('mois', $mois);//Place le mois de la fiche dans la SESSION

				$idVisiteur = $this->session->userdata('idUser');// Récupère l'idVisiteur de la SESSION ( Util co )
				
				$this->a_visiteur->MotifRefus($idVisiteur, $mois,$idFiche);
				// ... et on revient à mesFiches
				//$this->a_visiteur->mesFiches($idVisiteur, "La fiche $mois a été réfusée.");
			}
			
			elseif ($action == 'refuseFiche') 	// RefuserFiche demandé : on active la fonction signeFiche du modèle visiteur ...
			{	// TODO : contrôler la validité du second paramètre (mois de la fiche à modifier)
				...
				$idFiche = $this->session->userdata('idFiche');
				$mois = $this->session->userdata('mois');
				$idVisiteur = $this->session->userdata('idUser');
				
				$lemotif = $this->input->post('motif');
				
				
				$this->a_visiteur->refuseFiche($idVisiteur, $mois,$idFiche,$lemotif);
				// ... et on revient à mesFiches
				$this->a_visiteur->mesFiches($idVisiteur, "La fiche $mois a été réfusée.");
			}
