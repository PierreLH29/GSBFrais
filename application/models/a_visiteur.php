...
class A_visiteur extends CI_Model {
	...
	*/
	...
	/**
	 * Modifie une ligne de frais hors forfait dans une fiche donnée
	 *
	 * @param $idVisiteur : l'id du visiteur
	 * @param $mois : le mois de la fiche concernée
	...
	{	// TODO : s'assurer que les paramètres reçus sont cohérents avec ceux mémorisés en session
		...
		$this->dataAccess->AccepterFiche($idVisiteur, $mois,$idFiche);
		
	}
	
	/**
	 * Permet de gérer le motif de refus
	 *
	 * @param $idVisiteur : l'id du visiteur
	 * @param $mois : le mois de la fiche concernée
	 * @param $idFiche : l'id de la fiche concernée
	 */
	public function MotifRefus($idVisiteur, $mois,$idFiche)
	{
		
		$this->templates->load('t_comptable', 'v_motifRefus');
	}
	 ...
	 */
	public function refuseFiche($idVisiteur, $mois, $idFiche, $lemotif)
	{
		$this->dataAccess->refuserFiche($idVisiteur, $mois,$idFiche,$lemotif);
	}
}