...
// TODO : Transformer toutes les requêtes en requêtes paramétrées
	...
	*/
	...
	/**
	 * Permet d'accepter une fiche de frais
	 *
	 * @param $idVisiteur : l'id du visiteur
	 * @param $mois : le mois de la fiche concernée
	 * @param $idFiche : l'id de la fiche concernée
	 */
	public function AccepterFiche($idVisiteur, $mois,$idFiche){
		//met à 'VA' son champs idEtat
		$laFiche = $this->getLesInfosEtat($mois,$idFiche);
		
		if($laFiche['idEtat']=='CL'){
			$this->majEtatFicheFraisEtat($idFiche, $mois,'VA');
		}
	}
	
	/**
	 * Permet de refuser une fiche
	 *
	 * @param $idVisiteur : l'id du visiteur
	 * @param $mois : le mois de la fiche concernée
	 * @param $idFiche : l'id de la fiche concernée
	 * @param $lemotif : le motif de refus
	 */
	public function refuserFiche($idVisiteur, $mois,$idFiche,$lemotif){
		//met à 'RE' son champs idEtat
		$laFiche = $this->getLesInfosEtat($mois,$idFiche);
		
		if($laFiche['idEtat']=='CL'){
			$this->majEtatFicheFraisEtat($idFiche, $mois,'RE');
			$this->refuseMotif($mois, $idFiche,$lemotif);
		}
	}
	
	/**
	 * Permet de récupérer les ifnos dans la BDD
	 *
	 * @param $mois : le mois de la fiche concernée
	 * @param $idFiche : l'id de la fiche concernée
	 */
	public function getLesInfosEtat($mois,$idFiche){
		$req = "select ficheFrais.idEtat as idEtat, ficheFrais.dateModif as dateModif,
		ficheFrais.nbJustificatifs as nbJustificatifs, ficheFrais.montantValide as montantValide, etat.libelle as libEtat,
		ficheFrais.MotifRefus
		from  fichefrais inner join Etat on ficheFrais.idEtat = Etat.id
		where fichefrais.idvisiteur ='$idFiche' and fichefrais.mois = '$mois'";
		$rs = $this->db->query($req);
		$laLigne = $rs->first_row('array');
		return $laLigne;
	}
	...
	/**
	 * Permet de mettre a jour la fiche de frais
	 *
	 * @param $idVisiteur : l'id du visiteur
	 * @param $mois : le mois de la fiche concernée
	 * @param $idFiche : l'id de la fiche concernée
	 * @param $etat : l'etat
	 */
	public function majEtatFicheFraisEtat($idFiche,$mois,$etat){
		$req = "update ficheFrais
		set idEtat = '$etat', dateModif = now()
		where fichefrais.idvisiteur ='$idFiche' and fichefrais.mois = '$mois'";
		$rs = $this->db->query($req);
	...
	/**
	 * Permet de mettre a jour le motif de refus dans la bdd
	 *
	 * @param $mois : le mois de la fiche concernée
	 * @param $idFiche : l'id de la fiche concernée
	 * @param $lemotif : le motif de refus
	 */
	public function refuseMotif($mois, $idFiche,$lemotif){
		$req = "update ficheFrais
		set  MotifRefus = '$lemotif'
		where fichefrais.idvisiteur ='$idFiche' and fichefrais.mois = '$mois'";
		$rs = $this->db->query($req);
		$this->db->simple_query($req);
	}
